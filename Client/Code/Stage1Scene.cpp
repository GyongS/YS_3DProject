...
void CStage1Scene::CameraEvent()
{
	...
	if (m_EventCount < 1)
	{
		m_pMainCamera->GetTransform()->AddPositionX(1.f * GET_DT);
		m_EventTimer += GET_DT;
	}
	...
	if (m_pMainCamera->GetTransform()->GetRotation().x <= -35.f && m_EventCount < 1)
	{
		m_CameraMoveDelay += GET_DT;

		if (m_CameraMoveDelay > 1.f)
		{
			m_EventCount = 1;
