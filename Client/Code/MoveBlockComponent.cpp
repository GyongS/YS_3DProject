...
CMoveBlockComponent::CMoveBlockComponent() : m_speed(5.f)
{
}
...
void CMoveBlockComponent::Start(SHARED(Engine::CComponent) spThis)
{
...

	{
		...
		m_pObj->GetComponent<Engine::CTransformComponent>()->SetPosition(_float3(1, 8, 32));
...
void CMoveBlockComponent::RClicked()
{
	...
	transform->AddPosition(m_moveDir[m_moveType] * m_speed * GET_DT);
	...
	else if (Engine::CInputManager::GetInstance()->RayCast(transform->GetPosition(), m_moveDir[m_moveType], transform->GetScale().x * 0.5f, L"NormalBlock"))
	{
		--m_moveType;
		if (m_moveType == -1)
		{
