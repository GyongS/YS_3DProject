...
CStage2Scene::CStage2Scene() : m_NextSceneDelay(0.f)
{
}
...
void CStage2Scene::Start(void)
{
...

	{
		...
		pObj->GetComponent<Engine::CTransformComponent>()->SetPosition(_float3(-1, 8, 20));
		pObj->GetComponent<CMoveBlockComponent>()->SetGoalPos(_float3(-4, 8, 31));
	}
}
...
_uint CStage2Scene::Update(void)
{
	...
	if (m_pMainCamera->GetCameraEvent())
	{
		...
		m_pMainCamera->GetTransform()->AddPositionZ(1.f * GET_DT);
		m_pMainCamera->GetTransform()->SetRotationY(0.f);
		m_pMainCamera->GetTransform()->SetRotationX(-5.f);
