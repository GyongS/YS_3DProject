...
CElevator::CElevator() : m_speed(30.f)
{
}
...
void CElevator::ElevatorActivation()
{
	...
	for (auto gameObject : m_pQube)
	{
		...
		gameObject->GetComponent<Engine::CTransformComponent>()->Lerp(curPos, goalPos, m_speed * GET_DT);
		...
		m_playerObj->GetComponent<Engine::CRigidBodyComponent>()->SetVelocity(FLOAT3_ZERO);
		i++;
	}
	...
	{		
		Engine::GET_CUR_SCENE->SetShakeEvent(true);
		m_end = true;
	}
}